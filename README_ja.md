...
最も簡単なインストール方法は Homebrew を使用することです：
...
### Release からのプリビルトバイナリ

[Releases ページ](https://github.com/sinofseven/oidc-jwks-converter/releases)からお使いのプラットフォーム向けのプリビルトバイナリをダウンロードできます。以下のプラットフォームに対応しています：

- **Linux x86_64** (GNU および musl)
- **Linux ARM64**
- **Linux ARM**
- **macOS** (aarch64-apple-darwin)
- **Windows** (x86_64)

アーカイブを展開し、実行可能ファイルを PATH に配置します。

### crates.io を使用したインストール

Rust がインストール済みの場合、`cargo` でインストールできます：
...
ソースからビルドするには Rust 1.56 以上が必要です。リポジトリをクローンしてビルドします：
